...
use crate::types::evm_types::{
    EvmNativeTokenLastPrice, EvmNativeTokenLastPriceResponse, EvmNativeTokenLastPriceResult,
};
...
impl Stats {
    ...
    pub fn get_native_token_last_price(
        &self,
        ctx: &Context,
    ) -> Result<EvmNativeTokenLastPrice, EvmError> {
        let action = match ctx.chain {
            ChainType::BSC => "bnbprice",
            ChainType::BNBT => "bnbtprice",
            ChainType::Ethereum => "ethprice",
            ChainType::Polygon => "maticprice",
        };
        let raw_url_str = format!(
            "{}/api?module=stats&action={}&apikey={api_key}",
            Context::get_prefix_url(ctx.chain),
            action,
            api_key = ctx.api_key
        );
            ...
            .body(())
        {
            Ok(res) => res,
            Err(e) => {
                return Err(EvmError::ErrorInternalGeneric(Some(format!(
                    "Error creating a HTTP request; err={}",
                    e
                ))))
            }
        };
        ...
        match isahc::send(request) {
            Ok(mut res) => {
                ...
                if res.status() != 200 {
                    return Err(EvmError::ErrorApiResponse(format!(
                        "Error API resonse, with HTTP {code} returned",
                ...
                match res.json::<EvmNativeTokenLastPriceResponse>() {
                    Ok(json) => {
                        if json.status == "1" {
                            match json.result {
                                EvmNativeTokenLastPriceResult::Success(lastprice_struct) => {
                                    Ok(lastprice_struct)
                                }
                                EvmNativeTokenLastPriceResult::Success_Polygon(
                                    lastprice_struct,
                                ) => {
                                ...
                                EvmNativeTokenLastPriceResult::Failed(result_msg) => {
                                    Err(EvmError::ErrorApiResponse(format!(
                                        "un-expected error for success case ({msg})",
                                        msg = result_msg
                                    )))
                                }
                            }
